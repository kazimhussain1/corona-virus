...
app.get('/api/world_data', (req, res) => {
  const world_data = fs.readFileSync('corona-data.json');
  res.setHeader('Content-Type', 'application/json');
  res.send(world_data);
});

app.get('/api/mapTopoData', (req, res) => {
  const world_data = fs.readFileSync('world-map.json');

  res.send(world_data);
});

app.get('/api/news', (req, res) => {
  const world_data = fs.readFileSync('./data/news-data.json');
  res.setHeader('Content-Type', 'application/json');
  res.send(world_data);
});
...
//     run(res.data).catch(err => console.error(err));
//   })
//   .catch(err => {
//     console.log(err);
//   })
